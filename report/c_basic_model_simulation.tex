...
The two control law has been compared below for all the WS between the cut in and the rated one. Their difference is very slight, even though the controller based on the generator output power performs better.
\textcolor{red}{Add figure here}
The evaluation of the performance has been also done by computing the energy harvested in the two cases, by performing the integral of the power curve. Agin, the difference is not so big, but it is higher for the new controller: 
