...
In the Simulink model the implementation of the generator's torque reference $T_G^*$ is done employing a proportional and a saturation blocks, limiting the torque when it goes above the rated speed. Following what is presented in \cite{Olimpo_Anaya‐Lara}, an intermediate block with a \acrshort{PI} controller uses the error between the reference generator power $P_G^*$ and the actual one $P_G$ to set the generator torque. The control schema is presented in \autoref{fig:d_torque_control_2} \textcolor{red}{Sostituire al blocco PMSM la dicitura "Current control loop" per evidenziare il fatto che il riferimento di coppia non stia entrando direttamente nalla macchina ma nel suo anello di controllo di corrente }.
\begin{figure}[htb]
    \centering
...
\textcolor{red}{Menzionare nel testo la possibilità di incorrere in minimi locali durante la minimizzazione che possono portare a produrre dei risultati sbagliati}
