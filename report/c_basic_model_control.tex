...
\subsubsection[Implementation details]{Implementation details of the genereator's controller based on the rotor power}
In the Simulink model the implementation of the generator's torque reference $T_G^*$ is done employing a proportional gain and a saturation blocks. Following what is presented in \cite{Olimpo_Anaya‐Lara}, an intermediate block with a \acrshort{PI} controller uses the error between the reference generator mechanical input power $P_G^*$ and the actual one $P_G$ to set the generator torque. The control schema is presented in \autoref{fig:d_torque_control_2}.
\begin{figure}[htb]
    \centering
...
To study the problem, a numerical method is employed. For a reasonably fine sampling of velocities in the considered range, the inverse of the power function has been minimized (i.e. equivalent to maximizing the power itself) with respect to the pitch angle and the TSR. The results are shown in \autoref{fig:parameters_for_below_rated}, alongside the corresponding counterpart found with the policy based on the rotor power maximization. The study has been repeated twice, considering or excluding the transmission damping. In fact, as could be seen in \autoref{eq:P_G_out_eq}, when the damping is not included in the picture, the output power is the incoming in the rotor minus the joule loss (since in that case $T_R=T_G$).\\
An important assumption done here is that the rated WS is not changed with respect to the case of the $P_R$ maximization. This is done to make the comparison between the two control methods easier by identifying the same operating regions. This choice have an implication on the values of the nominal powers which requires a further explanation to avoid misunderstandings. In fact, during the definition of $V_{0,rated}$, after having found the optimal pitch angle and TSR, the power was computed with the cubic function of the WS, and then the velocity at which the rated power was reached identified. At this point, given the mechanical power, it was possible to retrieve the others (i.e. $P_G$ and $P_{GE}$). Now the steps followed are not exactly the same, since after having obtained the new pitch and TSR, the use of the power curve up to the previously rated WS lead to a $P_{GE}$ slightly different from the old one, and thus to different $P_G$ and $P_R$. \autoref{tab:table_power} summarizes these differences and may help to better understand them. 
\begin{table}
  \centering
...
Once the torque reference is synthesized according to the desired control law as described in \autoref{subsec:torque_reference} or \autoref{subsec:method_control_P_GE}, the first operation of the control scheme is to transform it in a $I_q \, \mesunt{\ampere}$ current reference knowing that:
\begin{equation}
