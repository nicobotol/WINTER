...
In \autoref{fig:fig_pitch_vs_V0} it is possible to see again some differences at low \acrshort{WSs} ($4\div7 \, \mesunt{\meter\per\second}$), which are related to the one of \autoref{subsec:lookup_cp}. Since operating at a fixed rotational speed in that range of \acrshort{WS} is not optimal because the \acrshort{TSR} is fixed at a value lower than the nominal, then the blades are pitched to obtain the highest $c_P$ allowed by the constraint on $\lambda$. Again this effect is not taken into account in the thesis and the pitch is considered to be 0 $\mesunt{\degree}$ below rated \acrshort{WS}.
