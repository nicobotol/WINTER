...
Before being applied on the \acrshort{WT} under study, the method is validated on the NREL5MW. The results of the validation and the calculation of the inertia are reported in \autoref{tab:rotor_inertia} alongside the percentage error computed as $\text{Error}=\frac{ |\text{Computed} - \text{Reference}| }{\text{Reference}}$.
...
\end{equation}
Even tough the nominal power of the turbine is $P_{rated} = 10 \left[\si{\mega \watt}\right]$, from the results presented in \cite{DTU_Wind_Energy_Report-I-0092} it could be seen that the mechanical power is slightly higher. This is reasonable because the turbines are usually identified by their electrical output power and not their incoming mechanical one. For this reason a reference value of $P_{rated} = 10.64 \left[\si{\mega \watt}\right]$ is considered during the computations. In order to compare the different results, the analysis are repeated twice, one for each of the two different $P_{rated}$: in \textit{Case 1} $P_{rated}=10 \ \left[\si{\mega \watt}\right]$ while in \textit{Case 2} $P_{rated}=10.64 \ \left[\si{\mega \watt}\right]$ and so the other values are recomputed by consequence. Furthermore, the \acrshort{TSR} from the DTU report is assumed to be 7.5 as design choice, while in the two computed cases they are the ones generating the maximum power coefficients, and are slightly higher (7.93 in the first case and 7.91 in the second). \\ 
...
In \autoref{fig:fig_pitch_vs_V0} it is possible to see again some differences at low \acrshort{WSs} ($4\div7 \, \mesunt{\meter\per\second}$), which are related to the one of \autoref{subsec:lookup_cp}. Since operating at a fixed rotational speed in that range of \acrshort{WS} is not optimal because the \acrshort{TSR} is fixed at a value lower than the rated, then the blades are pitched to obtain the highest $c_P$ allowed by the constraint on $\lambda$. Again this effect is not taken into account and the pitch is considered to be 0 $\mesunt{\degree}$ below rated \acrshort{WS}.
