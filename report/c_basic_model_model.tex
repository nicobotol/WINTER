...
\textcolor{red}{Questa sezione potrebbe non cominciare a pagina nuova, lo farò fare in seguito}
...
\subsection{Limitation of the scope}
\textcolor{blue}{Limitation of the scope moved here} Before going on with the modelling of the different sub-components, it is necessary to introduce the assumptions that have been done in order to simplify the analysis.
\begin{itemize}
  \item \textcolor{blue}{In this thesis the control is mainly focused is the power extraction, meaning that the aim is the conversion of most of the available power, from the resource or from the generator, according to different controls formulation, up to the rated power.} In real turbines other kinds of controller run alongside the power one, such as one for limiting the aerodynamic loads, the vibrations, contrasting the wave effect (in case of offshore plants) and providing ancillary services to the grid. The literature about controllers applied to wind turbines is very wide. In this thesis the main followed references are \cite{Olimpo_Anaya‐Lara}, \cite{NREL_5MW_reference} and \cite{DTU_Wind_Energy_E_0028}. 
...
\end{figure}
In this simple simulation, the transmission ratio is unitary, and so the relationship between therotational speed of the rotor $\omega_{R}$ and the generator $\omega_{G}$ is:
\begin{equation}
...
\autoref{fig:PMSM} represents the block diagram of q-axis current control of a \acrshort{PMSM}, coupled with also the mechanical equation. $R_{iq}$ is the controller, \acrshort{tauc} $\si{\second}$ is the time delay of the power electronic converter, $T_R \ \si{\newton\meter}$ is the torque imposed by the wind on the rotor.\\
...
A better visualization of the power conversion from the resource up to the generator output can be better visualized in \autoref{fig:d_sankey}. The representation considers a static condition at rated \acrshort{WS}, so the power lost during the transitory effects (i.e. stored/provided by the inertia during acceleration and deceleration of the rotor, and lost in the indiction of the generator) are not considered, and so their indications is written in italic. 
\begin{figure}[htb]
  \centering
  \input{d_sankey.tex}
  \caption{Sankey diagram of the power flow at the rated WS. The italic text are the powers lost during the transitory phases, and are not present in steady state.}
  \label{fig:d_sankey}
\end{figure}
