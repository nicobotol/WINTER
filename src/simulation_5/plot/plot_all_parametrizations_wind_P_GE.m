...
plot_parametrization_wind_P_GE('fig_power_param',out_store,'P_G',lookup_static_values(1,pos_4:pos_25),lookup_static_values(6,pos_4:pos_25)/1e6,'Wind speed [m/s]','P [MW]','Generator input power',1e6,simulation,date_fig)

% generator output power parametrization
plot_parametrization_wind_P_GE('fig_electrical_power_param',out_store,'P_GE',lookup_P_GE(1,:),lookup_P_GE(2,:)/1e6,'Wind speed [m/s]','P [MW]','Generator electrical power',1e6,simulation,date_fig)

% generator torque parametrization
plot_parametrization_wind_P_GE('fig_torque_param',out_store,'T_G', lookup_static_values(1,pos_4:pos_25), lookup_static_values(5,pos_4:pos_25)/1e6,'Wind speed [m/s]','T [MNm]', 'Generator torque',1e6,simulation,date_fig)

% pitch parametrization
plot_parametrization_wind_P_GE('fig_pitch_param',out_store,'pitch', lookup_Pitch(1,pos_4:pos_25),lookup_Pitch(3,pos_4:pos_25)*180/pi, 'Wind speed [m/s]','$\theta$ [deg.]','Pitch angle',pi/180,simulation,date_fig)
...
x_my_ref = cell(2);
y_my_ref = cell(2);
% x_my_ref{2} = lookup_static_values(1,pos_4:pos_25);
% y_my_ref{2} = lookup_static_values(2,pos_4:pos_25)*30/pi; 
x_my_ref{2} = lookup_static_values(1,pos_ramp_start:pos_ramp_stop);
y_my_ref{2} = lookup_static_values(2,pos_ramp_start:pos_ramp_stop)*30/pi; 
