function plot_parametrization_wind_zoom_P_GE(plot_name,out_cell,series,x_my_ref, y_my_ref,x_label,y_label,plot_title,scaling,simulation,date_fig)
  ...
  for i=1:wind.WS_len
    ...
    plot(out_cell{i}.wind.Data(s_start:end), out_cell{i}.(series).Data(s_start:end)/scaling, 'LineWidth', line_width, 'Color', colors_vect(i,:));
  ...
  [zoom_y_max] = out_cell{1}.(series).Data(zoom_end);
  ...
  line([max_zoom_V0 (axis_pos_x+axis_scale_x)*10], [zoom_y_max/1e6 (axis_pos_y+axis_scale_y)*12], 'Color', 'k')
