function energy = compute_comp_gains(out_store, simulation, IMM, wind, vector, name)
  ...
  for j=1:length(vector)
    ...
    t_start = simulation.stop_time(3) - simulation.post_process_time(3);
    [~, s_start] = min(abs(out_store{3}.P_GE.Time - t_start)); % element where to start the integration
    energy_ref_GE = trapz(out_store{3}.P_GE.Time(s_start:end), out_store{3}.P_GE.Data(s_start:end));
    energy_ref_R = trapz(out_store{3}.P_R.Time(s_start:end), out_store{3}.P_R.Data(s_start:end));
    ...
    for i=1:wind.WS_len
      fprintf(fileID,' & %d & %.3f & %.3f & %.3f & %.3f\\\\ \n', [i, energy_R(i)/1e9, diff_energy_R(i), energy_GE(i)/1e9, diff_energy_GE(i)]');
    end
