...
Pi = 0.99*eye(n_models, n_models) + 0.005*(ones(n_models, n_models)-eye(n_models)); % mode transition matrix
t_tot = 100; % number of time sample to investigate
R = 1; % measurement noise
Q = 0.1*eye(states_len); % process noise
mu = zeros(n_models);
model{1}.cp = [0.35];
% model{2}.cp = [0.38];
% model{3}.cp = 0.4;
V0 = 5; % wind speed
cp_real = 0.42; % intial cp
...
x_real = [0.9; 5]; % state
P_real = 10*eye(states_len); % covariance
x_est = x_real;
P_est = P_real;
...
for j = 1:n_models
  model{j}.x_est = [0.9; 5]; % state
  model{j}.P_est = 10*eye(states_len); % covariance
  model{j}.mu = 1/n_models; % mode probability
end

for t=1:t_tot
  pitch = 0; % pitch angle
  T_G = 0.5*cp_real*pi*rotor.R^2*V0^3/x_real(1); % torque generated by the generator

  % Propagate the dynamic
  x_real(2) = V0; % wind speed (ADD NOISE HERE)
  lambda =  rotor.R*x_real(1)/x_real(2); % TSR
  cp_real = min(0, interp2(lambda_vector, pitch_vector, lookup_cP, lambda, pitch));
  param{4} = cp_real;
  x_real = dynamic(x_real, T_G, param) + mvnrnd(zeros(states_len, 1), Q)' ; % propagate the dynamic and add noise
...
plot(x_real_store(1, :), 'DisplayName', 'real')
plot(x_est_store(1, :), 'DisplayName', 'estimation')
legend()
xlabel('Time')
ylabel('$\omega$')
...
function x_est = dynamic(x_est, T_G, param)
  ...
  x_est(1) = x_est(1) + 1/I_eq*(1/2*rho*pi*R^2*cp*x_est(1)^3*x_est(2)^(-1) - T_G);
  x_est(2) = x_est(2);
end
...
function matrices = compute_matrices(x_est, param)
  ...
  F = [1/I_eq*(3/2*rho*pi*R^2*cp*x_est(2)^2*x_est(1)^(-1)) 1-1/I_eq*(1/2*rho*pi*R^2*cp*x_est(2)^3*x_est(1)^(-2));
       1 0];
  H = [1 0];
  G = [1 1];
