...
V0_vector = V0_rated:1:25;
V0_len = length(V0_vector);
dPdt0_store = length(V0_vector);

theta_offset = 5*pi/180;  % angle range [rad]
delta_theta = 0.1*pi/180; % angular discretization [rad]

for v =1:V0_len
  V0 = V0_vector(v);
  % Define the parameters and the angle ranges used for the evaluations
  theta = interp1(lookup_Pitch(1,:), lookup_Pitch(3, :), V0); % pitch angle [rad/s]
  theta_vector = theta-theta_offset:delta_theta:theta+theta_offset;
  [min_tv, pos_min_tv] = min(abs(theta_vector - theta));
  if min_tv > fake_zero % Check if 0 [rad] is included
    theta_vector(end + 1) = theta;
  ...
  % rated condition
  lambda = rotor.R*omega_rated/V0;  % optimal TSR
...
[~, pos] = min(abs(dPdt0_store - 2*dPdt0_store(1)));
KK = interp1(lookup_Pitch(1,:), lookup_Pitch(3, :), V0_vector(pos)); % pitch angle [rad/s]
