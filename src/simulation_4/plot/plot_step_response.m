function plot_step_response(out_cell)
  ...
  plot(out_cell.T_G_reference.Time, out_cell.T_G_reference.Data, '--',...
    'LineWidth', line_width);
  hold on
  plot(out_cell.T_G.Time, out_cell.T_G.Data, 'LineWidth', line_width*0.75);
  ...
  plot(out_cell.T_G_reference.Time(N_start:N_end), ...
    out_cell.T_G_reference.Data(N_start:N_end), '--','LineWidth', 1.5, ...
    'MarkerSize',6, 'Color', colors_vect(1,:)); %axis tight
  hold on
  plot(out_cell.T_G.Time(N_start:N_end), ...
    out_cell.T_G.Data(N_start:N_end), 'LineWidth', 1.5, ...
